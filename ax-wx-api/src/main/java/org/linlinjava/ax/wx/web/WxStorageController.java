...
public class WxStorageController {
    ...
    private String generateKey(String originalFilename) {
        ...
        String key = null;
        AxStorage storageInfo = null;

        do {
            key = CharUtil.getRandomString(20) + suffix;
            storageInfo = axStorageService.findByKey(key);
        }
    ...
    public Object upload(@RequestParam("file") MultipartFile file) throws IOException {
        ...
        AxStorage axStorage = storageService.store(file.getInputStream(), file.getSize(), file.getContentType(), originalFilename);
    ...
    public ResponseEntity<Resource> fetch(@PathVariable String key) {
        AxStorage axStorage = axStorageService.findByKey(key);
        if (key == null) {
