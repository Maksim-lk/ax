...
import org.linlinjava.ax.wx.vo.CouponVo;
import org.linlinjava.ax.wx.util.WxResponseCode;
...
public class WxCouponController {
    ...
    public Object list(@RequestParam(defaultValue = "1") Integer page,
        ...
        List<AxCoupon> couponList = couponService.queryList(page, limit, sort, order);
    ...
    public Object mylist(@LoginUser Integer userId,
        ...
        List<AxCouponUser> couponUserList = couponUserService.queryList(userId, null, status, page, limit, sort, order);
        List<CouponVo> couponVoList = change(couponUserList);
        return ResponseUtil.okList(couponVoList, couponUserList);
    }

    private List<CouponVo> change(List<AxCouponUser> couponList) {
        List<CouponVo> couponVoList = new ArrayList<>(couponList.size());
        for(AxCouponUser couponUser : couponList){
            Integer couponId = couponUser.getCouponId();
            AxCoupon coupon = couponService.findById(couponId);
    ...
    public Object selectlist(@LoginUser Integer userId, Integer cartId, Integer grouponRulesId) {
        ...
        List<AxCouponUser> availableCouponUserList = new ArrayList<>(couponUserList.size());
    ...
    public Object receive(@LoginUser Integer userId, @RequestBody String body) {
        ...
        // 用户领券记录
        AxCouponUser couponUser = new AxCouponUser();
    ...
    public Object exchange(@LoginUser Integer userId, @RequestBody String body) {
        ...
        AxCoupon coupon = couponService.findByCode(code);
        if(coupon == null){
