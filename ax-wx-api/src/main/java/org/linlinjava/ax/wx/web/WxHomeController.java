...
public class WxHomeController {
    ...
    private List<Map> getCategoryList() {
        ...
        List<AxCategory> catL1List = categoryService.queryL1WithoutRecommend(0, SystemConfig.getCatlogListLimit());
        for (AxCategory catL1 : catL1List) {
            List<AxCategory> catL2List = categoryService.queryByPid(catL1.getId());
            List<Integer> l2List = new ArrayList<>();
            for (AxCategory catL2 : catL2List) {
                l2List.add(catL2.getId());
            }

            List<AxGoods> categoryGoods;
            if (l2List.size() == 0) {
