...
import org.linlinjava.ax.core.system.SystemConfig;
...
import static org.linlinjava.ax.wx.util.WxResponseCode.GOODS_NO_STOCK;
import static org.linlinjava.ax.wx.util.WxResponseCode.GOODS_UNSHELVE;
...
public class WxCartController {
    ...
    @Autowired
    private AxCartService cartService;
    @Autowired
    ...
    public Object index(@LoginUser Integer userId) {
        ...
        List<AxCart> cartList = cartService.queryByUid(userId);
        Integer goodsCount = 0;
        BigDecimal goodsAmount = new BigDecimal(0.00);
        Integer checkedGoodsCount = 0;
        BigDecimal checkedGoodsAmount = new BigDecimal(0.00);
        for (AxCart cart : cartList) {
     ...
     */
    @PostMapping("add")
    public Object add(@LoginUser Integer userId, @RequestBody AxCart cart) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        if (cart == null) {
        ...
        AxGoodsProduct product = productService.findById(productId);
        //判断购物车中是否存在此规格商品
        AxCart existCart = cartService.queryExist(goodsId, productId, userId);
        if (existCart == null) {
            //取得规格的信息,判断规格库存
     ...
     */
    @PostMapping("fastadd")
    public Object fastadd(@LoginUser Integer userId, @RequestBody AxCart cart) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        if (cart == null) {
     ...
     */
    @PostMapping("update")
    public Object update(@LoginUser Integer userId, @RequestBody AxCart cart) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        if (cart == null) {
        ...
        //判断是否存在该订单
        // 如果不存在，直接返回错误
        AxCart existCart = cartService.findById(id);
        if (existCart == null) {
    ...
    public Object checkout(@LoginUser Integer userId, Integer cartId, Integer addressId, Integer couponId, Integer grouponRulesId) {
        ...
        // 收货地址
        AxAddress checkedAddress = null;
        if (addressId == null || addressId.equals(0)) {
            checkedAddress = addressService.findDefault(userId);
            // 如果仍然没有地址，则是没有收货地址
            // 返回一个空的地址id=0，这样前端则会提醒添加地址
            if (checkedAddress == null) {
                checkedAddress = new AxAddress();
        ...
        AxGrouponRules grouponRules = grouponRulesService.queryById(grouponRulesId);
        if (grouponRules != null) {
            grouponPrice = grouponRules.getDiscount();
        }

        // 商品价格
        List<AxCart> checkedGoodsList = null;
        if (cartId == null || cartId.equals(0)) {
            checkedGoodsList = cartService.queryByUidAndChecked(userId);
        } else {
            AxCart cart = cartService.findById(cartId);
            if (cart == null) {
                return ResponseUtil.badArgumentValue();
            }
        ...
        for (AxCart cart : checkedGoodsList) {
        ...
        List<AxCouponUser> couponUserList = couponUserService.queryAll(userId);
        for(AxCouponUser couponUser : couponUserList){
            AxCoupon coupon = couponVerifyService.checkCoupon(userId, couponUser.getCouponId(), checkedGoodsPrice);
            if(coupon == null){
                continue;
            }
        ...
        if (couponId == null || couponId.equals(-1)){
            ...
            AxCoupon coupon = couponVerifyService.checkCoupon(userId, couponId, checkedGoodsPrice);
